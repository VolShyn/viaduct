...
Viaduct ships with a light but powerful **runtime plugin API**. You can inject new UI pieces, override existing ones, render global overlays or even access the core Zustand store—all without touching the core source.
...
Registrations **accumulate** rather than replace: a slot is shared, and `global-overlay` in particular is already claimed by `sequence-editor`, `docs-editor`, `data-flows` and `arch-tools`. Every registered node is rendered, in registration order. There is no removal API—a plugin that needs a dismissable overlay should register once and manage its own visibility internally.
