...
Cypress.Commands.add('addNode', () => {
  ...
  cy.get('[data-testid=toolbar-add-empty]').click();
...
Cypress.Commands.add('exportModel', () => {
  ...
  cy.get('[data-testid=toolbar-export-full]').click();
...
/**
 * Edit node name via the edit dialog (inline rename is disabled).
 */
Cypress.Commands.add('editNodeNameInline', (node, newName) => {
  node.find('[data-testid=block-edit-button]').click();
  cy.get('.MuiDialog-root').should('be.visible');
  cy.get('[data-testid=input_name]').clear().type(newName);
...
Cypress.Commands.add('editNodeProperty', (property: string, value: string) => {
  cy.get('.react-flow__node').first().find('[data-testid=block-edit-button]').click();
  cy.get('.MuiDialog-root').should('be.visible');
