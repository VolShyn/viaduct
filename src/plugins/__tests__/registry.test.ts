...
describe('PluginRegistry', () => {
  ...
  describe('Portal Registration', () => {
    ...
    it('should return null for non-existent portal', () => {
    ...
    // A slot is a multiplexer, not a single seat: `global-overlay` alone is
    // claimed by sequence-editor, docs-editor, data-flows and arch-tools, and
    // overwriting would silently drop every plugin but the last to boot.
    it('should accumulate portals registered under the same ID', () => {
      const portal1 = React.createElement('div', { testId: 'portal1' });
      const portal2 = React.createElement('div', { testId: 'portal2' });
      ...
      expect(registry.getPortal('portal')).toEqual([portal1, portal2]);
    });

    it('should unwrap a lone portal so a single registrant renders directly', () => {
      const portal = React.createElement('div', { testId: 'only' });

      registry.registerPortal('portal', portal);

      expect(Array.isArray(registry.getPortal('portal'))).toBe(false);
      expect(registry.getPortal('portal')).toBe(portal);
    });

    it('should preserve registration order across many registrants', () => {
      const portals = ['a', 'b', 'c'].map((testId) => React.createElement('div', { testId }));
      portals.forEach((portal) => registry.registerPortal('slot', portal));

      expect(registry.getPortal('slot')).toEqual(portals);
    });
  });
