...
describe('FileOperations', () => {
  ...
  describe('handleExportModel', () => {
    it('should export model and trigger download', () => {
      ...
      expect(mockExportModel).toHaveBeenCalledWith('full');
    ...
    it('should export current view with a distinct filename', () => {
      mockExportModel.mockReturnValue('{"view": true}');
      const mockAnchor = {
        href: '',
        download: '',
        click: jest.fn(),
      ...
      handleExportModel('current');

      expect(mockExportModel).toHaveBeenCalledWith('current');
      expect(mockAnchor.download).toBe('c4model-view.json');
    });
  });
